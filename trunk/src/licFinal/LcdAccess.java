...
public class LcdAccess implements KitConstants {
	...
	public static void setCursor(boolean visible, boolean blinking) {
		needRS = false;
		write((char) (visible ? CURSOR_ON_MASK
				| (blinking ? BLINK_ON_MASK : BLINK_OFF_MASK)
				: (CURSOR_OFF_MASK | (blinking ? BLINK_ON_MASK : BLINK_OFF_MASK))));
	}
	...
	public static void writeLine(int line, String txt) {
		posCursor(line, (isCentered) ? (DISPLAY_SIZE_MASK - txt.length()) / 2
				: 0);
		write(txt);
	}
